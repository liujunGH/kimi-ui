...
2026-08-29 官方 bundle 同步至 `@moonshot-ai/kimi-code@0.39.1`。bundle 体积行为下表所列数值；真机指标（导航、DOM、帧率、内存、合成回归）仍是 2026-08-06 在 `0.33.0` bundle 上测得的基线，升级后需按下方"每次升级的真实体验检查"重新测量后再更新。
...
| bundle | 532 个文件，34,121,324 bytes；入口 JS 3,145,130 bytes，入口 CSS 547,290 bytes（0.39.1 实测，入口 JS 距 3 MiB 预算余量 598 bytes） |
