...
- 修复"＋ 连接"对话框被标签栏裁剪、无法手动输入地址：标签栏 WebView 只有 36px 高，而对话框渲染在其下方视口外（此前仅有无障碍接口能填值，真实点击看不到输入框）。现在打开对话框时该 WebView 会临时增高，关闭后收回
- 标签支持右键重命名（多地址场景下便于区分）与右键关闭；重命名后的本地标签名称会持久化，远程标签连同令牌一并保存
- 连接持久化调整：本地连接只保存名称（地址与令牌每次从活动 daemon 重新获取），令牌仍只写在 0600 权限的文件里
